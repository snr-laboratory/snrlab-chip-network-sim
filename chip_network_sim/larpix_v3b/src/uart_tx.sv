...
module uart_tx
    ...
    (output logic tx_out,           // output bit sent on neg edge
...
always_ff @(posedge clk or negedge reset_n) begin
    if (!reset_n) begin
        ...
        if (tx_busy) begin
            tx_cnt <= tx_cnt + 1'b1;
            if (tx_cnt == 8'b0) tx_bit <= 1'b0;
            if (tx_cnt > 8'b0 && tx_cnt <= WIDTH) tx_bit <= tx_reg[tx_cnt - 8'd1];
...
// launch on negative edge to maximimze settling time
always_ff @(negedge clk or negedge reset_n)
    if (!reset_n) tx_out <= 1'b1;
    else tx_out <= tx_bit;
