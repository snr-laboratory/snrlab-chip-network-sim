...
```text
http://localhost:8000/sim_core/visualizers/packet_transmission/
```
...
The live 3x5 chip-14 event run now also has a trace-driven playback path:
- the live trace collector records exact chip tx_packet, rx_packet, and charge_injected events
- convert_live_trace_to_playback.py converts that JSONL trace into browser playback JSON
- the browser app highlights chips on charge_injected ticks and can load an event playback file through the playback query parameter or the file picker
