...
startup_in="$repo_root/sim_core/config/startup_3x5_event_top_right.json"
base_stimulus_json="$repo_root/sim_core/config/stimulus_3x5_event_top_right.json"
...
scenario_name="larpix_3x5_event_top_right"
...
python3 "$repo_root/sim_core/tools/generate_bootstrap_event_startup_json.py" \
  --rows 3 \
  --cols 5 \
  --s 0 \
  --target-x 4 \
...
python3 - "$startup_compiled" "$log_file" "$occupancy_csv" "$occupancy_png" "$occupancy_zoom_png" "$channel_generation_csv" "$channel_fifo_detail_csv" "$trace_jsonl" "$playback_json" "$repo_root/sim_core/tools/larpix_uart.py" <<'PY2'
import csv
...
print(f'visualizer_url_hint=http://localhost:8000/sim_core/visualizers/packet_transmission/?playback=/build/{pathlib.Path(playback_json).parent.name}/live_event_3x5_chip14.json')
PY2
