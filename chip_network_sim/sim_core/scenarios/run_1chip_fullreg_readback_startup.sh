...
# - build/chip_larpix_v3b exists and runs with the real analog + Verilated digital core
...
startup_in="$repo_root/sim_core/config/startup_1chip_full_reg_readback.json"
rtl_variant="${RTL_VARIANT:-v3b}"
...
scenario_name="larpix_1chip_readback_smoke"
...
case "$rtl_variant" in
  v3b)
    constants_file="$repo_root/rtl/v3b/src/larpix_constants.sv"
    assign_file="$repo_root/rtl/v3b/src/config_regfile_assign.sv"
    ;;
  v3b_v2)
    constants_file="$repo_root/rtl/v3b_v2/src/larpix_constants.sv"
    assign_file="$repo_root/rtl/v3b_v2/src/config_regfile_assign.sv"
    ;;
  v3c)
    constants_file="$repo_root/rtl/v3c/src/larpix_constants.sv"
    assign_file="$repo_root/rtl/v3c/src/config_regfile_assign.sv"
    ;;
  *)
...
python3 "$repo_root/sim_core/tools/generate_1chip_full_reg_readback_json.py"   --constants "$constants_file"   --assign "$assign_file"   --out "$startup_in"

python3 "$repo_root/sim_core/tools/compile_startup_json.py"   "$startup_in"   "$startup_compiled"
...
"$build_dir/orchestrator_larpix"   -rows 1   -cols 1   -ticks "$ticks"   -source_x 0   -source_y 0   -chip_bin "$build_dir/$chip_bin_name"   -fpga_bin "$build_dir/fpga_larpix"   -startup_json "$startup_compiled"   -trace_out "$trace_jsonl"   > "$log_file" 2>&1

python3 - "$log_file" "$repo_root/sim_core/tools/larpix_uart.py" "$repo_root/sim_core/tools/rtl_config_defaults.py" "$constants_file" "$assign_file" <<'PY2'
