...
#
# Test intent:
# - instantiate a 15-row by 15-column LArPix network with source chip (8,6)
...
work_dir="$build_dir/larpix_15x15_event_staggered_multichip_source_8_6"
...
python3 "$repo_root/larpix_network_sim/visualizers/packet_transmission/convert_live_trace_to_playback.py" \
  ...
  --out "$playback_json" \
  --rtl-version "v3b" \
  --name "15x15 RTL-Preconfigured Two-Track Staggered Multi-Chip Injection (source 8,6)"
...
print('visualizer_url_hint=http://localhost:8000/larpix_network_sim/visualizers/packet_transmission/?playback=/build/larpix_15x15_event_staggered_multichip_source_8_6/live_event_15x15_staggered_multichip.json')
PY2
