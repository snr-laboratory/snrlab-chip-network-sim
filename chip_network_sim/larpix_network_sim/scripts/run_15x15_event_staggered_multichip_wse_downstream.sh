...
# and staggered multi-chip injections across multiple final chip IDs, with
# alternate downstream routing on injected chips.
#
# Test intent:
...
# - then override injected chips so downstream TX enables West, South, and East
...
work_dir="$build_dir/larpix_15x15_event_staggered_multichip_wse_downstream"
...
playback_json="$work_dir/live_event_15x15_staggered_multichip_wse_downstream.json"
...
WSE_DOWN_MASK = 0xE
target_runtime_ids = {48, 64, 80, 96, 112, 113, 129, 145, 183, 184, 169, 155, 141}
path = Path(sys.argv[1])
raw = json.loads(path.read_text())
for entry in raw.get('register_writes', []):
    runtime_id = int(entry.get('runtime_id', -1))
    register_addr = int(entry.get('register_addr', -1))
    if runtime_id in target_runtime_ids and register_addr == ENABLE_PISO_DOWN_REG:
        entry['register_data'] = WSE_DOWN_MASK
        entry['label'] = f'override downstream TX mask to WSE for runtime {runtime_id}'
for entry in raw.get('targets', []):
    if int(entry.get('runtime_id', -1)) in target_runtime_ids:
        entry['down_mask_override'] = WSE_DOWN_MASK
...
pathlib.Path(sys.argv[1]).write_text(
    '// 15x15 analog/cosim preconfigured two-track staggered multi-chip event test stimulus with WSE downstream override\n'
...
python3 "$repo_root/larpix_network_sim/visualizers/packet_transmission/convert_live_trace_to_playback.py" \
  ...
  --out "$playback_json" \
  --rtl-version "v3b" \
  --name "15x15 RTL-Preconfigured Two-Track Staggered Multi-Chip Injection (WSE Downstream Override)"
...
print('visualizer_url_hint=http://localhost:8000/larpix_network_sim/visualizers/packet_transmission/?playback=/build/larpix_15x15_event_staggered_multichip_wse_downstream/live_event_15x15_staggered_multichip_wse_downstream.json')
PY2
