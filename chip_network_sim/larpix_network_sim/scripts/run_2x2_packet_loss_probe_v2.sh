...
work_dir="$build_dir/larpix_2x2_packet_loss_probe_v2"
init_json="$work_dir/init_2x2_packet_loss_probe_v2_preconfigured.json"
stimulus_json="$work_dir/stimulus_2x2_packet_loss_probe_v2.json"
log_file="$work_dir/run.log"
trace_jsonl="$work_dir/trace.jsonl"
playback_json="$work_dir/live_event_2x2_packet_loss_probe_v2.json"
...
print(injection_tick, injection_tick + 10000)
PYT
)"
...
python3 "$repo_root/larpix_network_sim/visualizers/packet_transmission/convert_live_trace_to_playback.py" \
  ...
  --out "$playback_json" \
  --rtl-version "v3b2" \
  --name "2x2 RTL-Preconfigured Packet Loss Probe (v2 alternate RTL)"
...
python3 - "$trace_jsonl" "$log_file" "$summary_json" "$repo_root/larpix_network_sim/scripts/larpix_uart.py" <<'PYSUM'
...
summary = {
    'scenario': '2x2 RTL-Preconfigured Packet Loss Probe (v2 alternate RTL)',
    'source_runtime_id': 0,
