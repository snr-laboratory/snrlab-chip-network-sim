...
# 3x3 live-network msg_rtl probe with one runtime MSG_OP enable write and
# center-chip analog charge injection.
#
...
# - injects one startup CONFIG_WRITE to chip_id 4 setting DMONITOR0[5]=1
# - injects analog charge into channels 0..31 of runtime/chip 4
# - summarizes whether chip 4 emits MSG_OP packets to its neighbors

repo_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
build_dir="$repo_root/build"
work_dir="$build_dir/larpix_3x3_msg_center_charge_probe"
init_json="$work_dir/init_3x3_msg_center_charge_probe_preconfigured.json"
startup_in_json="$work_dir/startup_3x3_msg_center_charge_probe.json"
startup_compiled_json="$work_dir/startup_3x3_msg_center_charge_probe_compiled.json"
stimulus_json="$work_dir/stimulus_3x3_msg_center_charge_probe.json"
log_file="$work_dir/run.log"
trace_jsonl="$work_dir/trace.jsonl"
rx_debug_csv="$work_dir/runtime4_rx_debug.csv"
summary_json="$work_dir/msg_center_charge_probe_summary.json"
            ...
            "frames": [
                {
                    ...
                    "register_addr": 114,
                    "register_data": 0x20,
                    "label": "enable msg generation on chip 4 via DMONITOR0[5]",
                }
            ]
        },
        indent=2,
...
pathlib.Path(sys.argv[1]).write_text(
    "// 3x3 msg_rtl center-chip probe stimulus\n"
...
for line in trace_jsonl.read_text().splitlines():
    ...
    if event_name == "tx_packet":
        ...
        if packet_type == MSG_OP:
            ...
            reg_addr = (word >> 10) & 0xFF
            reg_data = (word >> 18) & 0xFF
            if runtime_id == center_runtime and reg_addr == 114 and reg_data == 0x20:
                chip4_config_write_seen += 1
        elif packet_type == CONFIG_READ_OP:
            entry["config_read"] += 1
...
summary = {
    "scenario": "3x3 msg center charge probe",
    ...
    "config_write_to_center_msg_enable_seen": chip4_config_write_seen,
