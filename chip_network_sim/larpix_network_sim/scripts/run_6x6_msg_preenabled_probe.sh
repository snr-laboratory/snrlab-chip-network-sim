...
# - uses no analog charge injection by default
# - generates playback so the initial neighbor-state dissemination can be inspected

repo_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
build_dir="$repo_root/build"
work_dir="$build_dir/larpix_6x6_msg_preenabled_probe"
init_json="$work_dir/init_6x6_msg_preenabled_probe_preconfigured.json"
stimulus_json="$work_dir/stimulus_6x6_msg_preenabled_probe.json"
log_file="$work_dir/run.log"
trace_jsonl="$work_dir/trace.jsonl"
rx_debug_csv="$work_dir/chip0_rx_debug.csv"
summary_json="$work_dir/msg_preenabled_probe_summary.json"
run_metrics_json="$work_dir/run_metrics.json"
playback_json="$work_dir/live_event_6x6_msg_preenabled_probe.json"
ticks="${TICKS:-2000}"
...
pathlib.Path(sys.argv[1]).write_text(
    "// 6x6 msg_rtl pre-enabled probe stimulus\n"
...
python3 "$repo_root/larpix_network_sim/visualizers/packet_transmission/convert_live_trace_to_playback.py" \
  ...
  --name "6x6 msg pre-enabled probe"
...
lane_state_updates_by_chip = {str(k): 0 for k in range(36)}
...
for line in trace_jsonl.read_text().splitlines():
    ...
    if event_name == "lane_state" and runtime_id >= 0:
        lane_state_updates_by_chip[str(runtime_id)] += 1
        continue
...
summary = {
    "scenario": "6x6 msg pre-enabled probe",
    "msg_enable_register_addr": 114,
    "msg_enable_register_data": 32,
    "msg_enable_target_chip_ids": list(range(36)),
    "msg_tx_by_chip": msg_tx_by_chip,
    "msg_rx_by_chip": msg_rx_by_chip,
    "lane_state_updates_by_chip": lane_state_updates_by_chip,
