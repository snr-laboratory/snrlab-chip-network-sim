...
chip_network_sim/
  ...
  sim_core/include/chipsim/
    protocol.h         # Active control/data-plane structs
    trace.h            # Active trace/event schema
  sim_core/src/
  ...
  sim_core/tools/
    *.py               # Startup compilers, JSON generators, UART helpers
  sim_core/scenarios/
    run_*.sh           # Runnable active scenarios
  sim_core/config/
    *.json             # Startup and stimulus assets
  rtl/
    v3b/
    v3b_v2/
    v3c/
...
### 4.1 RTL-backed active path
- The active branch builds RTL-backed chip executables from `rtl/*/src`.
- `sim_core/src/chip_larpix.cpp` is the process wrapper around the Verilated `digital_core`.
- `sim_core/src/larpix_cosim_backend.cpp` drives RTL signals per tick and exchanges data/control over `nng`.

### 4.2 Common transport
...
- Summary:
  - legacy trace reconstruction is no longer part of the active branch workflow
- ASCII packet columns:
  - use the active browser playback flow under `sim_core/visualizers/packet_transmission/`
